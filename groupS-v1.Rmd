...
df01%>% 
  ggplot(aes(x = year, y = regional_avg,alpha=0.8,
                   color = factor(superregion2,levels=c("East & Southeast Asia",
                                           "Former Soviet Union", 
                                           "High-Income Countries", 
                                           "Latin America & Caribbean",
...
world_cancer <- read.csv("data/world_cancer_2018.csv")
...
#试了与原来的life_df合并，下面用tmap画图也还是报错
combined <- life_sp@data %>%
  left_join(world_cancer,by=c(ISO3="Code"))
