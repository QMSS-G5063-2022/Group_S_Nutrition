...
packages <- c("readr","ggplot2","tidyverse","dplyr","data.table","fs","plotly",
            ...
            'gridExtra','rio','textstem','tm','qdapRegex','wordcloud')
...
What aspects are people concerned about nutritional intake in the world?     

Here we generate a word cloud from the recent 7 days tweets which is under the tag "#nutritional" through Twitter API, and we would like to see whether those texts refer to specific types of food that we can pay more attention to later.
...
The wordcloud covers various types of things from drinks like soda to foods like tomato and meat. It cannot lead to a specific type for us to investigate later.
...
annotate_figure(nvrg_gender,top = text_grob("Dietary Structure by Gender in China", 
                                color = "tomato", face = "bold", size = 14))
```
...
First, we must figure out whether there are some spatial similarities in life expectancy around the world.
